...
st.markdown(
    """
            Website App to perform webscraping of NBA player stats data!
...
selectedYear = st.sidebar.selectbox(
    "YEAR", list(reversed(range(1950, 2022)))
...
def loadStats(year):
    url = (
        ...
        + str(year)
        + "_per_game.html"
    )  # This url is no longer valid, we need to address the right one
    html = pd.read_html(
        url, header=0
    )  # To easily read the data as table inside the url
    df = html[0]
    row = df.drop(df[df.Age == "Age"].index)  # Deletes repeating indexes of Age
    row = row.fillna(0)  # Allows to fill missing values
